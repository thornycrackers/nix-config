...
# Sometimes there are resolution failures, sleep for a couple seconds
sleep 2

# Install/Enable ssh server for ansible
incus exec "$container_name" -- yum update -y
incus exec "$container_name" -- yum install -y openssh-server
incus exec "$container_name" -- systemctl enable sshd
incus exec "$container_name" -- systemctl start sshd
...
incus exec "$container_name" -- usermod -aG wheel ansible
incus exec "$container_name" -- /bin/bash -c 'echo "%wheel ALL=(ALL) NOPASSWD:ALL" >>/etc/sudoers'
