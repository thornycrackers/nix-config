...
gwu() {
    ...
    local FILES=($(git -C $ROOT_DIR ls-files --others --exclude-standard| fzf --multi --reverse))
    for FILE in ${FILES[@]}; do
        rm "$ROOT_DIR/$FILE"
    done
}
...
gbxb() {
    local BRANCHES=($(git branch | cut -c 3- | fzf --multi --reverse))
...
gbcc() {
    local BRANCH=($(git branch | cut -c 3- | fzf))
    git checkout $BRANCH
}
...
gf() {
    ...
    if [[ "$2" == "master" ]]; then
        2=$(git name-rev --name-only master)
    fi
    git fetch "$1" "$2"
}
